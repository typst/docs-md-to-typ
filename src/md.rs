...
fn convert_heading(node: &mdast::Heading, ctx: &mut Context) -> String {
    ...
    let patched = replace(
        ...
        |cap: &regex::Captures| {
            ...
            let mut body = long.to_string();
            let label = match cap.get(3) {
                Some(id) => {
                    let id = id.as_str();
                    let short = if id.starts_with('v') && id.contains('.') {
                        id.into()
                    } else {
                        id.to_title_case()
                    };
                    ctx.imports.insert("short-or-long");
                    body = format!("#short-or-long[{short}][{long}]");
                    id.to_string()
                }
                None => long.to_kebab_case(),
            };
